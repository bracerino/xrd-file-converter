...
Online tool for converting X-ray diffraction data files between formats (.xrdml, .ras, .raw, .xy) and between different X/Y axis (different wavelengths, d-spacing, q-spacing, automatic - divergence slits), for merging/averaging multiple .xy XRD data into a single pattern, for interactively plotting and stacking multiple measured patterns, and for digitizing curves from an image or a photo of a plot back into .xy data.
